...
**Three actions per row.** Deploy the branch, restart, and open the app in Hatchbox. Deploy and restart ask for confirmation first. While the job is queued, the whole row pulses in the accent colour. Once it is running, that row's rocket (or restart glyph) rattles and takes the theme accent, or orange when the accent would not stand out, pixel exhaust streams out of the rocket's tail, and thin streaks pass around it like stars past a ship. When the job ends the effect tapers off over a second. The panel polls the job until it settles, so a failed deploy turns solid red without a refresh. Opening the panel refreshes, so a deploy Hatchbox started from a git push shows the same way. A restart only speaks for the row while it is running or has failed: a restart that completes does not clear a failed deploy, because the failing build is still what's live.
